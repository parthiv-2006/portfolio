...
function computeStreaks(days) {
    ...
    // Collect streak dates while walking backwards
    ...
    const current = streakDates.size;
    ...
    return { current, longest, todayCount, lastActive, streakDates };
}
...
// 28-day activity strip — the unique data viz element
function CommitStrip({ contributions, streakDates, numDays = 28 }) {
    const today = daysAgo(0);
    ...
    return (
        <div className="flex gap-[3px] w-full">
            {cells.map(({ date, count, inStreak, isToday }) => (
                <div
                    key={date}
                    title={`${count} commit${count !== 1 ? 's' : ''} · ${date}`}
                    className="flex-1 aspect-square rounded-[2px] cursor-default transition-opacity duration-150 hover:opacity-80"
                    style={{
                        background: inStreak && count > 0
                            ? 'var(--color-accent)'
                            : count > 0
                            ? 'rgba(226,160,78,0.18)'
                            : 'rgba(255,255,255,0.04)',
                        outline: isToday ? '1.5px solid rgba(226,160,78,0.45)' : 'none',
                        outlineOffset: '1.5px',
                    }}
                />
            ))}
...
export default function GitHubStreak({ compact = false }) {
    ...
    if (compact) {
        return (
            ...
            >
                {loading ? (
                    <div className="h-9 w-64 rounded-lg bg-surface-light/30 animate-pulse" />
                ) : (
                    <div className="inline-flex items-center gap-3 px-4 py-2.5 rounded-lg border border-white/[0.06] bg-surface/60">
                        ...
                        <div className="flex gap-[2px]">
                            {Array.from({ length: 14 }, (_, i) => {
                                ...
                                const count = map[date] ?? 0;
                                const inStreak = streakDates.has(date);
                                return (
                                    <div
                                        key={date}
        ...
        >
            ...
            {loading ? (
                <div className="space-y-4">
                    ...
                    <div className="h-5 w-full rounded bg-surface-light/20 animate-pulse" />
                </div>
            ) : (
                <>
                    {/* Stats row */}
                    <div className="flex items-end justify-between mb-5 gap-4">
                        {/* Current streak — the number does the talking */}
                        <div>
                        ...
                        {/* Secondary stats — right-aligned, small */}
                        <div className="flex items-end gap-5 pb-0.5">
                            <div className="text-right">
                                ...
                                <p className="font-mono text-[10px] text-text-dim uppercase tracking-[0.1em] mt-1">
                                    best
                                </p>
                            </div>
                            {todayCount > 0 && (
                    ...
                    {/* 28-day commit strip — the unique visual */}
                    <CommitStrip contributions={data} streakDates={streakDates} numDays={28} />

                    {/* Strip axis labels */}
                    <div className="flex items-center justify-between mt-1.5">
                        <span className="font-mono text-[9px] text-text-dim">28d ago</span>
